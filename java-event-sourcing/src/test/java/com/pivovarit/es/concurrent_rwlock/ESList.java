...
public class ESList<T> implements List<T> {
    ...
    private final ReadWriteLock opLogLock = new ReentrantReadWriteLock();
    ...
    public int version() {
        opLogLock.readLock().lock();
        try {
            return opLog.size();
        } finally {
            opLogLock.readLock().unlock();
        }
    }
    ...
    private Object handle(ListOp<T> op) {
        opLogLock.writeLock().lock();
        List<T> snapshot = snapshot();
        try {
            opLog.add(op);
        } finally {
            opLogLock.writeLock().unlock();
        }
